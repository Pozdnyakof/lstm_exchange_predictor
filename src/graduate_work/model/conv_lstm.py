...
долгосрочные зависимости, а голова выдаёт мульти-горизонтный прогноз.

Архитектура одинакова для обоих режимов:
- regression:     выход = нормализованная лог-доходность на горизонт.
- classification: выход = ЛОГИТЫ; сигмоида применяется в инференсе.
В обоих случаях форма (B, num_horizons).
"""
